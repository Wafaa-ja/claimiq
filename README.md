...
ClaimIQ estimates expected motor insurance claim frequency from four fitted models. The Negative Binomial GLM's dispersion parameter (α) is estimated from the data via maximum likelihood, not fixed by assumption:

| Model | Type | Test MAE | AIC |
|-------|------|----------|-----|
| Poisson GLM | Statistical | 0.09888 | 229,548.83 |
| Negative Binomial GLM (NB2, α=0.9846) | Statistical | 0.09944 | 228,839.46 |
| Random Forest | Machine Learning | 0.09809 | N/A |
| XGBoost | Machine Learning | 0.09821 | N/A |

All four models were also validated across 20 independent 80/20 train-test splits to confirm the ranking above is reproducible, not an artifact of one split: Random Forest won 18/20 splits (90%), XGBoost 2/20 (10%), and neither GLM won a single split — a small but statistically reproducible predictive edge for the machine learning models. Full per-model mean/std/confidence-interval/win-rate results are in `data/repeated_split_summary.csv` and surfaced on the app's Model Comparison page. Every metric shown in the app is sourced from `data/research_results.json` (generated by `generate_research_results.py`), not hardcoded.
